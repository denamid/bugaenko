  #В КОНЦЕ УЖЕ ПОНЯЛ. ЧТО МОГ СОЗДАТЬ РАЗНЫЕ ПЕРЕМЕННЫЕ И ИМ НАЗНАЧИТЬ ФУНКЦИОНАЛ, А ПОТОМ УЖЕ ЧЕРЕЗ ЦИКЛЫ ВЫЗЫВАТЬ ОПРЕДЕЛЕННУЮ ПЕРЕМЕННУЮ ИЛИ ВСЕ СРАЗУ
#ЧТО БЫ ВЫЗВАТЬ СРАЗУ ВСЕ ПЕРЕМЕННЫЕ И ПОКАЗАТЬ РЕЗУЛЬТАТЫ ВСЕГО ФУНКЦИОНАЛА СРАЗУ, ТАК БЫЛО БЫ ЛАКОНИЧНЕЕ, ЧИТАБЕЛЬНЕЕ И ТД, ТУПОНУЛ, НО В СЛЕДУЮЩИЙ РАЗ БУДУ ТАК ДЕЛАТЬ. И этим убрать возможные еще ошибки
	#Ну и тут все через циклы, но и тема ведь циклы, так что все вроде четко. Так же мог легко убрать ошибку, если вводить слишком большое число, но это тоже понял только вот как дописал
print("Введите два числа для анализа диапазона")
x = input("Начало диапазона: ")
y = input("Конец диапазона: ")
if (x).lstrip("-""+").isdigit() == False or (y).lstrip("-""+").isdigit() == False : #Убираем ошибки, если вводят значения + или -, или же буквы, заствляя вводить по новой значение
...
nomer = int(input())
while True:        #Цикл для того, что бы можно было перевыбирать опции меню, и заканчивать цикл при выборе 6 - пунката
	while True:      #Циакл для  того что бы пользователь выбирал правильный функционал, от 1 до 6, при выборе больше, пишет что выбрали не правильный функционал, и выбираете заново
	...
	if  nomer == 1:
		...
		if l == "1":
			...
			i = 0
			while 1 == 1:                         #Выводим циклом много похвальный слов)))))
				print("Красавичик, ты лучший, молодец, возращайся, у тебя все получится, Life is Good, ты неповторимый")
				i = i + 1
				if i == 300000:
					break
			exit()              #В КОНЦЕ УЖЕ ПОНЯЛ. ЧТО МОГ СОЗДАТЬ РАЗНЫЕ ПЕРЕМЕННЫЕ И ИМ НАЗНАЧИТЬ ФУНКЦИОНАЛ, А ПОТОМ УЖЕ ЧЕРЕЗ ЦИКЛЫ ВЫЗЫВАТЬ ОПРЕДЕЛЕННУЮ ПЕРЕМЕННУЮ ИЛИ ВСЕ СРАЗУ
		                    #ЧТО БЫ ВЫЗВАТЬ СРАЗУ ВСЕ ПЕРЕМЕННЫЕ И ПОКАЗАТЬ РЕЗУЛЬТАТЫ ВСЕГО ФУНКЦИОНАЛА СРАЗУ, ТАК БЫЛО БЫ ЛАКОНИЧНЕЕ, ЧИТАБЕЛЬНЕЕ И ТД, ТУПОНУЛ, НО В СЛЕДУЮЩИЙ РАЗ БУДУ ТАК ДЕЛАТЬ, ЕСЛИ ЧТО